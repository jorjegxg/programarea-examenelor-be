...
namespace API.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateExamRequest([FromBody] ExamRequest examRequest)
        {
            ...
            return CreatedAtAction(nameof(GetExamRequest), new { id = examRequest.RequestID }, examRequest);
        }

        // GET: api/ExamRequests/5
        [HttpGet("{id}")]
        public async Task<IActionResult> GetExamRequest(int id)
        {
            var examRequest = await _context.ExamRequests.FindAsync(id);
